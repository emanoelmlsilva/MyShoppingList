...
class ChoiceDataFieldViewModel(context: Context, val lifecycleOwner: LifecycleOwner) :
    BaseFieldViewModel() {
    ...
    fun updateMonthCollection() {
        viewModelScope.launch {
            ...
            if (yearCurrent.value.isNotBlank() && dateMonthAndYear.value.isNotEmpty()) {
                val monthOfYearCollection =
                    if (dateMonthAndYear.value.size > 1) dateMonthAndYear.value[yearCurrent.value]!!.toList() else dateMonthAndYear.value.values.first()
                        .map { it.trim() }
    ...
    fun updateMonths(idCard: Long) {
        viewModelScope.launch {
            ...
            if (idCard == -1L) {
                val mothsAndYearCollection =
                    SeparateDateUtils.separateMonthAndYear(FormatDateUtils().getMonthYearAllAtCurrent())
                delay(100L)
